#!/usr/bin/env python3
"""Data loader for combined quality metrics."""
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent.parent))
from depanalysis.db_manager import DatabaseManager

def get_quality_overview(repo_name: str) -> dict:
    """Get combined quality metrics."""
    # Point to data directory in repo root (not docs/)
    data_dir = Path(__file__).parent.parent.parent.parent / "data"
    db_manager = DatabaseManager(data_dir=data_dir)
    if not db_manager.repo_exists(repo_name):
        return {"error": f"Repository '{repo_name}' not found"}

    try:
        # Get structural metrics
        conn_structure = db_manager.get_connection(repo_name, "structure")
        cursor_structure = conn_structure.cursor()

        # Complexity metrics
        complexity_stats = cursor_structure.execute("""
            SELECT
                AVG(cyclomatic_complexity) AS avg_complexity,
                MAX(cyclomatic_complexity) AS max_complexity,
                SUM(CASE WHEN cyclomatic_complexity > 15 THEN 1 ELSE 0 END) AS high_complexity_count,
                COUNT(*) AS total_functions
            FROM functions
            WHERE cyclomatic_complexity > 0
        """).fetchone()

        # Type safety metrics
        type_stats = cursor_structure.execute("""
            SELECT
                COUNT(DISTINCT f.id) AS total_functions,
                COUNT(DISTINCT CASE WHEN th.id IS NOT NULL THEN f.id END) AS typed_functions
            FROM functions f
            LEFT JOIN type_hints th ON th.function_id = f.id
        """).fetchone()

        # Instability metrics
        instability_stats = cursor_structure.execute("""
            SELECT
                AVG(instability) AS avg_instability,
                SUM(CASE WHEN instability > 0.8 AND afferent_coupling > 5 THEN 1 ELSE 0 END) AS high_risk_modules
            FROM instability
        """).fetchone()

        conn_structure.close()

        # Get temporal metrics
        conn_history = db_manager.get_connection(repo_name, "history")
        cursor_history = conn_history.cursor()

        # Churn metrics
        churn_stats = cursor_history.execute("""
            SELECT
                AVG(change_count) AS avg_changes,
                MAX(change_count) AS max_changes
            FROM churn_metrics
        """).fetchone()

        conn_history.close()

        # Calculate quality scores (0-100 scale)
        total_funcs = complexity_stats[3] or 1
        typed_funcs = type_stats[1] or 0
        high_complexity = complexity_stats[2] or 0

        complexity_score = max(0, 100 - (complexity_stats[0] or 0) * 5)  # Lower is better
        type_safety_score = (typed_funcs / total_funcs) * 100 if total_funcs > 0 else 0
        instability_score = max(0, 100 - (instability_stats[0] or 0) * 100)

        # Overall quality score (weighted average)
        overall_score = (
            complexity_score * 0.3 +
            type_safety_score * 0.3 +
            instability_score * 0.4
        )

        return {
            "repository": repo_name,
            "scores": {
                "overall": round(overall_score, 1),
                "complexity": round(complexity_score, 1),
                "type_safety": round(type_safety_score, 1),
                "instability": round(instability_score, 1)
            },
            "complexity_metrics": {
                "avg_complexity": round(complexity_stats[0] or 0, 2),
                "max_complexity": complexity_stats[1] or 0,
                "high_complexity_count": high_complexity,
                "total_functions": total_funcs
            },
            "type_metrics": {
                "typed_functions": typed_funcs,
                "total_functions": total_funcs,
                "coverage_percent": round((typed_funcs / total_funcs * 100) if total_funcs > 0 else 0, 1)
            },
            "instability_metrics": {
                "avg_instability": round(instability_stats[0] or 0, 3),
                "high_risk_modules": instability_stats[1] or 0
            },
            "churn_metrics": {
                "avg_changes": round(churn_stats[0] or 0, 2),
                "max_changes": churn_stats[1] or 0
            }
        }
    except Exception as e:
        return {"error": str(e)}

if __name__ == "__main__":
    repo_name = sys.argv[1] if len(sys.argv) > 1 else None
    if not repo_name:
        # Point to data directory in repo root (not docs/)
        data_dir = Path(__file__).parent.parent.parent.parent / "data"
        db_manager = DatabaseManager(data_dir=data_dir)
        repos = db_manager.list_analyzed_repos()
        repo_name = repos[0] if repos else None
    print(json.dumps(get_quality_overview(repo_name or ""), indent=2) if repo_name else json.dumps({"error": "No repos"}))
